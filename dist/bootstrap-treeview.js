...
(function ($, window, document, undefined) {
  ...
  Tree.prototype._setInitialStates = function (node, level) {
    this._nodes = {};
    return $.when.apply(this, this._setInitialState(node, level)).done(
      $.proxy(function () {
  ...
  Tree.prototype._sortNodes = function () {
    return $.map(
      Object.keys(this._nodes).sort(function (a, b) {
        if (a === b) return 0;
  ...
  Tree.prototype._setSelected = function (node, state, options, fired) {
    ...
    if (state) {
      ...
      if (!this._options.multiSelect) {
        $.each(
          this._findNodes("true", "state.selected"),
          $.proxy(function (index, node) {
            this._setSelected(
              node,
              false,
              $.extend(options, { unselecting: true }),
              true
            );
          }, this)
        );
  ...
  Tree.prototype._setChecked = function (node, state, options) {
    ...
    if (this._options.highlightChanges) {
      node.$el.toggleClass(
        "node-check-changed",
        (this._checkedNodes.indexOf(node) == -1) == state
      );
    }
  ...
  Tree.prototype._render = function () {
    if (!this._initialized) {
      ...
      this.$element.empty().addClass(pluginName).append(this.$wrapper);
  ...
  Tree.prototype._renderNode = function (node, previousNode) {
    ...
    if (!node.$el) {
      node.$el = this._newNodeEl(node, previousNode).addClass(
    ...
    node.$el.append(
      node.nodes || node.lazyLoad
        ? this._template.icon.expand.clone()
        : this._template.icon.empty.clone()
    );
    ...
    if (this._options.showTags && node.tags) {
      $.each(
        node.tags,
        $.proxy(function addTag(id, tag) {
          node.$el.append(
            this._template.badge
              .clone()
              .addClass(
              ...
              .append((typeof tag === "object" ? tag.text : undefined) || tag)
          );
        }, this)
      );
    }
  ...
  Tree.prototype._addCheckbox = function (node) {
    if (
      ...
      (node.hideCheckbox === undefined || node.hideCheckbox === false)
    ) {
      node.$el.append(this._template.icon.check.clone());
    }
  };
  ...
  Tree.prototype._addIcon = function (node) {
    if (this._options.showIcon && !(this._options.showImage && node.image)) {
      var icon = this._template.icon.node
        .clone()
        .addClass(node.icon || this._options.nodeIcon);
      if (node.iconColor) {
  ...
  Tree.prototype._expandNode = function (node) {
    ...
    $.each(
      node.nodes.slice(0).reverse(),
      $.proxy(function (index, childNode) {
  ...
  Tree.prototype._buildStyle = function () {
    ...
    if (
      ...
      (this._options.searchResultColor || this._options.searchResultBackColor)
    ) {
      var innerStyle = "";
      ...
      if (this._options.searchResultBackColor) {
        innerStyle +=
          "background-color:" + this._options.searchResultBackColor + ";";
      }

      style += ".node-" + this._elementId + ".node-result{" + innerStyle + "}";
      style +=
        ".node-" + this._elementId + ".node-result:hover{" + innerStyle + "}";
    }
    ...
    if (
      ...
      style +=
        ".node-" + this._elementId + ".node-selected{" + innerStyle + "}";
      style +=
        ".node-" + this._elementId + ".node-selected:hover{" + innerStyle + "}";
    }
    ...
    if (this._options.highlightChanges) {
      ...
      style +=
        ".node-" + this._elementId + ".node-check-changed{" + innerStyle + "}";
    }
  ...
  Tree.prototype.addNode = function (nodes, parentNode, index, options) {
    ...
    this._setInitialStates({ nodes: this._tree }, 0).done(
      $.proxy(function () {
  ...
  Tree.prototype.search = function (pattern, options) {
    ...
    $.each(
      this._diffArray(results, previous),
      $.proxy(function (index, node) {
    ...
    // Set new results
    $.each(
      this._diffArray(previous, results),
      $.proxy(function (index, node) {
